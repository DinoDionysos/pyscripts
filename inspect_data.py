...
for i in range(0, len(k_list)):
    ...
    axs[i//4, i%4].hist(list_slam_concat_ape[0], 
                        bins = k_list[i], density=True, 
                        histtype='stepfilled', 
                        label=names_of_slams[0], 
                        linewidth=2, alpha=1.0, 
                        color=np.array([0.2, 0.2, 0.2, 1.0]))
    axs[i//4, i%4].hist(list_slam_concat_ape[1], 
                        bins = k_list[i], density=True, 
                        histtype='stepfilled', label=names_of_slams[1], linewidth=2, alpha=0.5, 
    ...
    axs[i//4, i%4].hist(list_slam_concat_ape[1], 
                        ...
                        edgecolor='darkgreen')
    axs[i//4, i%4].hist(list_slam_concat_ape[2], 
                        bins = k_list[i], density=True, 
                        histtype='step', label=names_of_slams[2], 
