...
# caption
latex_string_diff += "\caption{Each column shows the comparison of two \\ac{slam} approaches with %d p-values of \\ac{%s} tests on \\ac{%s} data.}\n" % (num_of_tests_per_slam_combi, test_names[test_idx], error_type)
# label
...
for i in range(0, len(folders)):
    ...
    list_stat_p = hypothesis_test_list(columns_ape[i][:d], columns_ape[i][d:], test_names[test_idx])
    count_fail = sum([1 for i in list_stat_p if i < alpha])
    count_reject = len(list_stat_p) - count_fail
...
# caption
latex_string_self += "\caption{Each column shows the comparison of a \\ac{slam} approache with itself with %d p-values of \\ac{%s} tests on \\ac{%s} data.}\n" % (num_of_tests_per_slam, test_names[test_idx], error_type)
# label
