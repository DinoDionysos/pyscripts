...
from util_latex_tables_3 import *
...
for scenario in scenarios:
    ...
    df = df_latex_table_template_3(test_names, correlation_names)
    ...
    df = insert_multicol_at(df, 'Significance level $\\alpha$ = '+str(alpha)+", 20 repetitions", 1)
    slam_idx = -1
    for slam_idx_1 in range(0,len(folders)):
        for slam_idx_2 in range(0, slam_idx_1):
            ...
            caption = "Scenario %s: Tests on significant difference between monocular, stereo and RGBD ORB-SLAM. The table shows the counts of rejections and fails of rejection of 20 repetitions of the hypothesis tests \\ac{ks2} and Kendall's Tau with significance level 0.05. The tests were applied on the translational and rotational \\ac{ape}." % (
            ...
            save_name = scenario+"_compact"+".tex"
            if save_flag:
                save_latex_table(
                    latex_string_table, 
                    folder_save, 
